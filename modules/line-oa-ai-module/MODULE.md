...
* 💾 **Pluggable Session Storage:** ระบบจัดการประวัติการสนทนา (Chat History) และ State ผู้ใช้ ผ่าน `SessionStore` interface (get/set/delete/clear) พร้อม Auto TTL — มาพร้อม `MemorySessionStore` ในตัว, backend อื่น (Postgres/Redis/ฯลฯ) implement เพิ่มเองได้โดยไม่ต้องแก้ core (ดูแผนใน modules-hub `line-oa-ai-module/DESIGN.md` §persistent-session-store)
...
3. **Session Expiration:** กำหนด `sessionTtlMs` ให้เหมาะสมเพื่อคืน Memory หรือ implement `SessionStore` ด้วย backend ที่ persist ได้เองเมื่อขยายระบบเป็น Multi-instance
...
- **Core module มี `SessionStore` implementation เดียวคือ `MemorySessionStore`** (เก็บใน memory ของ process เดียว — restart แล้วข้อมูลหาย, scale เป็นหลาย instance ไม่ได้) — เดิม MODULE.md เคยเขียนว่ามี `RedisSessionStore` มาด้วย ไม่จริง แก้ข้อความแล้ว (2026-08-20) ดูแผนปรับปรุงใน modules-hub `line-oa-ai-module/DESIGN.md`
